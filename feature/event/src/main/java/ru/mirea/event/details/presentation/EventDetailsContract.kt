...
data class EventDetailsState(
    ...
    val optimizedDebts: List<OptimizedDebt> = emptyList(),
    val filteredOptimizedDebts: List<OptimizedDebt> = emptyList(),
    val transactions: List<Transaction> = emptyList(),
    val membersItems: List<Any> = emptyList(),
    val debts: List<DetailsDebt> = emptyList(),
    val filteredDebts: List<DetailsDebt> = emptyList(),
    val isLoading: Boolean = false,
    val error: String? = null,
    val showOnlyMine: Boolean = false,
    val myOweSum: Int = 0,
    val oweToMeSum: Int = 0,
)
...
sealed interface EventDetailsEvent {
    ...
    data class ShowOnlyMineChanged(val value: Boolean) : EventDetailsEvent
}
